...
- 在“生成剧情选项”前也可以手动添加剧情选项，输入选项和可选结果后直接进入当前选项池。
...
- `选项前生成正文` 控制选中分支后是否先生成下一段剧情正文，再进入下一轮选项；关闭后会把已选分支作为上下文种子，直接生成下一组选项。
...
- 首页和编辑页头部都可以进入“素材抽取”页面，作为当前项目的附属工作区使用。
- 素材抽取工作台采用两步流程：先执行“赛道分析”生成并确认样文赛道，再执行“DNA抽取”生成可复用资产。
- 工作台内部分为“赛道分析”和“DNA数据抽取”两个 tab，左侧结构会随当前 tab 切换为赛道产物或 DNA 资产库。
- 赛道分析 tab 左侧的赛道产物支持点击预览，右侧可查看样文、赛道分析 JSON、赛道结构卡和待分拣样文内容。
- 赛道分析结果按 `02_样文与拆解/` 路径管理；DNA 资产按 `04_DNA资产库/` 下的 L1 高频原料、L2 结构机制、L3 深度参考分组解析，资产关系网络位于 L2 首位。
- 赛道分析只返回赛道判断、结构特征和证据；实际样文路径与资产路径统一由前端基于已确认赛道和故事名解析，不再保存带 `[track_id]` 这类占位符的模板路径。
- 素材抽取结果按项目独立保存，包含源标题、源文本、赛道分析、确认赛道、结构化分析结果、提取摘要和资产总数。
- 清空素材抽取数据时，只会清空当前项目下的素材抽取记录，不影响其他项目。

对应实现：

- 页面入口与切换：`src/App.vue`
- 素材抽取页面：`src/features/materialExtractor/MaterialExtractorPage.vue`
- 素材抽取持久化：`server/index.js`、`server/db.js`、`src/services/databaseApi.js`
