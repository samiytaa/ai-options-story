...
`脑洞组装工坊` 当前运行在 `React + Vite` 前端宿主之上，并通过兼容桥挂载既有 `Vue` 工作台逻辑；它是一个互动式小说创作工作台，用来把“风向标 + 起始脑洞 + 分支选择”逐步组装成完整剧情，再按指定文风生成最终正文。

它现在不是单纯的一页原型，而是一个带本地项目、收藏库、提示词配置、右侧助手和 SQLite 持久化的创作工作台。

## 功能概览

补充说明：编辑页正文生成区域的详细功能拆解与操作 SOP 见 [docs/正文生成区域说明.md](docs/正文生成区域说明.md)。
...
- 选定脑洞后，流程会按“生成导语 -> 生成架构 -> 生成人设 -> 生成第一个剧情点”推进。
- `生成架构` 阶段默认按 9 到 12 章短篇规划，当前优先产出 10 章架构。它会先整理：
  `故事起点`、`核心冲突`、`10章事件链`、`前四章执行蓝图`、`第4章末付费点`、`付费后立即回报`、`后段大反转`、`反转铺垫`。
- 当前 app 的互动流程仍先执行前四章：前四章执行蓝图会约束第一到第四章的剧情点、章节钩子和第四章大钩子；第四章大钩子会作为第5章到第10章后续细纲的入口。
- `付费点位置` 现在固定为 `第4章末`，不再在架构页单独编辑。
...
- 服务端启动时会自动把历史项目快照里的旧版架构字段归一化到当前结构，避免旧字段继续写回数据库。
...
- 故事DNA数据库新增“例文库”tab，用于沉淀和管理项目内例文，支持两种创建方式：手动逐篇录入，以及批量上传 TXT 文件；同时支持编辑、删除、搜索、查看关联索引。
- 赛道分析和 DNA 数据抽取的输入区都支持“从例文库选择”，可把例文库中的正文直接载入当前分析区。
- 在赛道分析或 DNA 抽取中保存的正文会自动沉淀到例文库，并建立与赛道分析结果、DNA 抽取结果以及具体资产记录的关联索引。
- 故事DNA页面顶部提供 `提示词` 入口，用于管理赛道分析和 DNA 抽取提示词。
- 赛道归类支持两条路径：手动输入赛道会直接确认或创建赛道；自动判断只会归入当前已有赛道，未命中时弹出警告弹窗，由用户选择已有赛道、采用 AI 参考赛道名创建新赛道，或暂不归类。
- 用户创建的新赛道会持久化到独立赛道目录，后续项目和后续自动判断都会把它当作已有赛道参与归类。
- 素材抽取页新增“管理赛道”入口，可查看全部已有赛道；默认赛道只读，自定义赛道支持重命名，且仅在未被任何项目确认使用时允许删除。重命名或删除后，自动判断赛道时使用的“当前已有赛道”列表会立即同步为最新目录。
...
- 赛道分析会额外沉淀“脑洞生成规则包”，包括赛道定位、读者情绪需求、核心爽点/虐点、常见人设关系、章节节奏骨架、禁忌雷区和可组合变量标签，供后续抽脑洞、立项和细纲复用。
...
- React 宿主入口：`src/main.jsx`、`src/RootApp.jsx`
- Vue 兼容桥：`src/legacy/LegacyVueBridge.jsx`
- 现有工作台主入口：`src/App.vue`
